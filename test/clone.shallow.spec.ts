...
describe(`clone.shallow() - @category Object`, () => {
  ...
  describe(`should return clone of Date object`, () => {
    it(`Date() => Date()`, () => {
      ...
      clone.shallow(d).should.deep.equal(d);
    });
  });

  describe(`should return clone of RegExp`, () => {
    it(`/abc/ => /abc/`, () => {
      clone.shallow(/abc/).should.deep.equal(/abc/);
    });
  });
  ...
  describe(`should return clone of ArrayBuffer`, () => {
    it(`new ArrayBuffer(8) => new ArrayBuffer(8)`, () => {
      ...
      vi1[0] = 0xFF;
      vi1[1] = 0xEE;
      const ab2: ArrayBuffer = clone.shallow(ab1);
  ...
  describe(`should return clone of DataView`, () => {
    it(`new DataView() => new DataView()`, () => {
      ...
      dv1.setInt16(0, 0xFF);
      dv1.setInt16(2, 0xEE);
      const dv2: DataView = clone.shallow(dv1);
  ...
  describe(`should return clone of Typed Array as view of ArrayBuffer`, () => {
    it(`Typed Array view => Typed Array view`, () => {
      ...
      vi1[0] = 0xFF;
      vi1[1] = 0xEE;
      const vi2: Int32Array = clone.shallow(vi1);
  ...
  describe(`should return clone of Set`, () => {
    ...
    it(`new Set() => new Set()`, () => {
      ...
      clone.shallow(s1).should.deep.equal(s1);
    });
  ...
  describe(`should return clone of array`, () => {
    ...
    it(`[1,2,3] => [1,2,3]`, () => {
      clone.shallow([1, 2, 3]).should.deep.equal([1, 2, 3]);
    });
  ...
  describe(`should return clone of deeply nested array`, () => {
    ...
    it(`[1,[2,[3,[4]]]] => [1,[2,[3,[4]]]]`, () => {
      ...
      const arr2: any[] = clone.shallow(arr1);
      arr2.should.not.equal(arr1);
      arr2.should.deep.equal(arr1);
      arr2[1].should.equal(arr1[1]); // sahllow reference copy
      arr2[1][1].should.equal(arr1[1][1]); // sahllow reference copy
      arr2[1][1][1].should.equal(arr1[1][1][1]); // sahllow reference copy
  ...
  describe(`should return clone of plain object`, () => {
    ...
    it(`{a:1,b:2} => {a:1,b:2}`, () => {
      clone.shallow({ a: 1, b: 2 }).should.deep.equal({ a: 1, b: 2 });
    });
  ...
  describe(`should return clone of nested object`, () => {
    ...
    it(`{a:1,b:{c:2}} => {a:1,b:{c:2}}`, () => {
      clone.shallow({ a: 1, b: { c: 2 } }).should.deep.equal({ a: 1, b: { c: 2 } });
    });

  });

  describe(`should handle cyclic object with shallow reference copy`, () => {

    const a: object = { a: 1, b: { c: 'd' } };
    const b: object = clone.shallow(a);

    it(`{a:1, b:{c:'d'}} => new {a:1, b:{c:'d'}}`, () => {
      b['a'].should.equal(1);
      b['b']['c'].should.deep.equal('d');
      b['b'].should.equal(a['b']); // shallow reference cloning
    });

    a['f'] = a;
    a['g'] = a;
    const c: object = clone.shallow(a);

    it(`{a:1,b:{c:'d'},f:a,g:a} => new {a:1,b:{c:'d'},f:a,g:a}`, () => {
      c['a'].should.equal(1);
      c['b']['c'].should.deep.equal('d');
      c['b'].should.equal(a['b']); // shallow reference cloning

      c['f'].should.equal(a['f']); // shallow reference cloning
      c['f'].should.equal(c['g']);

      c['f']['f'].should.equal(c['f']);
      c['f']['g']['f'].should.equal(c['g']);
      c['f']['g']['f']['g']['f']['g'].should.not.equal(c); // non deep cyclic clone 
  ...
  describe(`should return {} for promise`, () => {
    it(`Promise.resolve(123) => {}`, () => {
      clone.shallow(Promise.resolve(123)).should.deep.equal({});
    });
  });

  describe(`should return {} for function`, () => {
    it(`()=>123 => {}`, () => {
      clone.shallow(() => 123).should.deep.equal({});
    });
  });

  describe(`should return {} for symbol`, () => {
    it(`Symbol() => {}`, () => {
      clone.shallow(Symbol()).should.deep.equal({});
    });
  });

  describe(`should return NaN as NaN`, () => {
    it(`NaN => NaN`, () => {
      clone.shallow(NaN).should.be.NaN;
    });
  });

  describe(`should return null as null`, () => {
    it(`null => null`, () => {
      should().equal(clone.shallow(null), null);
    });
  });

  describe(`should return undefined as undefined`, () => {
    it(`undefined => undefined`, () => {
      should().equal(clone.shallow(undefined), undefined);
    });
  });
  ...
  describe(`should be functional and not mutating any input`, () => {
    ...
    it(`[{a:1},{b:2}] => [{a:1}, {b:2}]`, () => {
      ...
      const output: any = clone.shallow([{ a: 1 }, { b: 2 }])
